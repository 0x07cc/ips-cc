        ...
        for riga2 in lista2:
            ...
            if trovato is not None:
                ...
                try:
                    services['O-'+str(porta)]=Service(porta,self.regex_list['O-'+str(porta)],self.services_type[porta],self.log,firewall_direction)
                        ...
                        if('I-'+str(porta) in str(e)):
                            #KILLA TUTTO
                            self.log.uplog("Can't find IPTables rule for "+str(porta)+" in "+firewall_direction,1,"red") #TODO Eng
                            exit(-1)
                            #services['O-'+str(porta)]=Service(porta,None,self.services_type[porta],self.log,firewall_direction)
                        elif(str(porta) in str(e)):
                            self.log.uplog("Can't find service type of "+str(porta)+" in "+firewall_direction+" ... setted as Raw by deafult",1,"yellow",1) #TODO Eng
                ...
                if trovato is not None:
                    ...
                    log.uplog("Qualcosa non va...regola IPtables sbagliata o porta non specificata?")
